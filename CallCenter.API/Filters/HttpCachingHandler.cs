using CallCenter.API.CasheServices;
    ...
    public class HttpCachingHandler: DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if(request.Method == HttpMethod.Get)
            {
                ...
                if (anyEtagsFromtheClientExist)
                {
                    ...
                    {
                        if(eTags.Any(_=>_.Tag == cacheableEntity.EntityTag.Tag))
                        {
            ...
            {
                if(request.Method == HttpMethod.Get && !_eTagCacheDictionary.TryGetValue(resourceKey, out cacheableEntity))
                {
                ...
                if (request.Method == HttpMethod.Post || request.Method == HttpMethod.Put || request.Method == HttpMethod.Delete)
                {
                    ...
                    CacheInvalidationStore.ForEach(func => func(GetRequestUri(request.RequestUri))
                    ...
                    invalidCaches.ForEach(invalidCacheUri =>
                    {
                        var cacheEntityKeys = _eTagCacheDictionary.Keys.Where(
                            x => x.StartsWith(string.Format("{0}:", invalidCacheUri), StringComparison.InvariantCultureIgnoreCase));
                ...
                else
                {
                    ...
                    _varyHeaders.ForEach(varyHeader => response.Headers.Vary.Add(varyHeader));
                }
            }
            return response;
        }
        ...
        private string GetResourceKey(string trimedRequestUri, string[] varyHeaders, HttpRequestMessage request)
        {
            var requestedVaryHeaderValuePairs = request.Headers
                .Where(_ => varyHeaders.Contains(_.Key))
                .Select(_ => string.Format("{0}:{1}", _.Key, string.Join(";", _.Value)));

            return string.Format("{0}:{1}", trimedRequestUri, string.Join("_", requestedVaryHeaderValuePairs)).ToLower(CultureInfo.InvariantCulture);
        }
        ...
        {
            return String.Concat(requestUri.LocalPath.TrimEnd('/'), requestUri.Query).ToLower(CultureInfo.InvariantCulture);
        }
    }
}